...
app.get('/google-drive/enabled', function () {
    return XlreConfig.getKeyValue('googleDrive.enabled');
});
...
var REDIRECT_URL = 'http://localhost:' +  XlreConfig.getServerPort() + '/google-drive/oauth2callback',
...
XlreApi.prototype.getOauth2 = function () {
    return new OAuth2(XlreConfig.getKeyValue('googleDrive.clientId'), XlreConfig.getKeyValue('googleDrive.clientSecret'), REDIRECT_URL);
};
    ...
    XlreSharedApi.findSnapshotByName(fileToDownload.title)
        .then(function (document) {
            progress(request({
                ...
                console.log("Downloading " + (state.received / document.size * 100).toFixed(2) + " %");

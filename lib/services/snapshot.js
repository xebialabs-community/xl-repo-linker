...
XlreSnapshot.prototype.getSnapshotFolders = function () {
    return Object.keys(XlreConfig.getSnapshotConfiguration());
};
...
XlreSnapshot.prototype.create = function () {
    ...
    function getSnapValueByKey(snapKey) {
        return snapConfig[snapKey].split(',');
    }
    ...
    _(folderLibs).forEach(function (folderLib) {
        ...
        if (fs.existsSync(fullFolderPath)) {
            bulkData.push({expand: true, cwd: fullFolderPath, src: getSnapValueByKey(folderLib), dest: folderLib});
        }
    }).value();

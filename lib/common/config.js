...
XlreConfig.prototype.enrichConfig = function (dbConfig) {

    var xldLicensePath = XlreConfig.prototype.fetchKeyValue(dbConfig, 'xld.license');

    if (fs.existsSync(xldLicensePath) && fs.lstatSync(xldLicensePath).isFile()) {
        XlreConfig.prototype.updateModelKeyValue(dbConfig, 'xld.license', xldLicensePath);
    }
...
XlreConfig.prototype.fetchKeyValue = function (dbConf, key) {
    var tokens = key.split('.');
    var confGroup = tokens[0];
    var confKey = tokens[1];
...
XlreConfig.prototype.getKeyValue = function (key) {
    var dbConf = XlreConfig.prototype.getXlreConfig();
    return XlreConfig.prototype.fetchKeyValue(dbConf, key);
};
